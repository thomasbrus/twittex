defmodule Twittex.Client do
  ...
  def search(query, options \\ []) do
    get "/search/tweets.json?" <> URI.encode_query(Keyword.merge([q: query], options))
  end
  ...
  def user_timeline(screen_name, options \\ []) do
    get "/statuses/user_timeline.json?" <> URI.encode_query(Keyword.merge([screen_name: screen_name], options))
  end
  ...
  * `:max_demand` - Maximum demand for this subscription.
  ...
  def stream(query \\ :sample, options \\ []) do
    ...
    url =
      case query do
        :user   -> "https://userstream.twitter.com/1.1/user.json?" <> URI.encode_query(Keyword.merge([delimited: "length"], options))
        :sample -> "https://stream.twitter.com/1.1/statuses/sample.json?" <> URI.encode_query(Keyword.merge([delimited: "length"], options))
        _       -> "https://stream.twitter.com/1.1/statuses/filter.json?" <> URI.encode_query(Keyword.merge([track: query, delimited: "length"], options))
      end
